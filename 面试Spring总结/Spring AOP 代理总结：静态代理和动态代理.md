...
==运行时增强==：动态代理就是说AOP框架==不会去修改字节码==，而是在==内存中临时为方法生成一个AOP对象==，这个AOP对象包含了目标对象的全部方法，并且在特定的切点做了增强处理，并回调原对象的方法。
...
<mark>不需要接口，只有实现类</mark>



<font color="red">**!!!在运行时创建目标类的子类(代理类)，从而对目标类进行增强!!!**</font>
